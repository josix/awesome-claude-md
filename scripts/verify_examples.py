...
def extract_github_links(file_path: str) -> dict[str, str]:
    ...
    repo_pattern = (
        r"\[(?:Repository|Source)\]\((https://github\.com/([^/]+)/([^/)]+))\)"
    )
...
def verify_directory_name(
    example_dir: Path, repo_owner: str, repo_name: str
) -> tuple[bool, str]:
...
def main():
    ...
    results = {"total": len(example_dirs), "passed": 0, "failed": 0, "issues": []}
    ...
    for example_dir in example_dirs:
        ...
        if not has_doc:
            issues.append("❌ Missing README.md or analysis.md")
        else:
            ...
            if not links["repo_url"]:
                issues.append("❌ No GitHub repository link found")
            else:
                print(f"  ✓ Repository: {links['repo_url']}")

            if not links["claude_md_url"]:
                issues.append("⚠️  No CLAUDE.md link found (may be optional)")
            else:
            ...
            if links["repo_owner"] and links["repo_name"]:
                name_match, name_issue = verify_directory_name(
                    example_dir, links["repo_owner"], links["repo_name"]
                )
                if not name_match:
                    issues.append(f"❌ Directory name mismatch: {name_issue}")
                else:
                    print("  ✓ Directory name matches repository")

        if issues:
            results["failed"] += 1
            results["issues"].append(
                {"path": example_path, "category": category, "issues": issues}
            )
            for issue in issues:
                print(f"  {issue}")
        else:
